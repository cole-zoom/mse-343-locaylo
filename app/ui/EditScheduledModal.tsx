...
export const EditScheduledModal: React.FC<EditScheduledModalProps> = ({ 
  isOpen, 
  onClose, 
  activityId 
}) => {
  ...
  const [status, setStatus] = useState<'pending' | 'scheduled'>('scheduled');
  ...
  useEffect(() => {
    if (activityId && isOpen) {
      const foundActivity = getActivity(activityId);
      if (foundActivity) {
        setActivity(foundActivity);
        setStatus(foundActivity.status);
        setDate(foundActivity.date);
        setStartTime(foundActivity.startTime);
        setEndTime(foundActivity.endTime);
      }
    }
  }, [activityId, isOpen, getActivity]);

  if (!isOpen || !activity) return null;

  const handleSave = () => {
    if (activityId) {
      updateActivity(activityId, { status, date, startTime, endTime });
      onClose();
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="flex gap-3">
          <button
            onClick={handleSave}
            className="flex-1 py-4 rounded-3xl text-lg font-black transition-all active:scale-[0.98]"
            style={{ 
